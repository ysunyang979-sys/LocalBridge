...
export function registerProjectTools(server: McpServer, nexusClient: NexusClient) {
  ...
  server.tool(
    "nexus_project_list",
    "列出当前 Nexus LocalBridge 中所有已登记并受控的本地项目，包含项目 ID、名称及状态信息",
    {},
    async () => {
      try {
  ...
  server.tool(
    ...
    {
      projectId: z.string().describe("项目名称或项目 ID，例如 'Myweb' 或 'proj_xxx'"),
    },
    async ({ projectId }) => {
      try {
        ...
        return {
          content: [{ type: "text", text: `Error fetching project info: ${err.message}` }],
          isError: true,
        };
      }
    }
