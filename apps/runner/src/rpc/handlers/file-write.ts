  ...
  return async (params: FileWriteParams): Promise<FileWriteResult> => {
    ...
    if (evalResult.decision === "deny") {
      ...
      throw new LocalBridgeError(
        ...
        evalResult.reason || `Operation "file.write" denied by policy.`
      );
    }
    ...
    if (evalResult.decision === "ask") {
      if (!approvalManager) {
        throw new LocalBridgeError(
          ...
          `Operation "file.write" requires human approval.`
        );
      }
      ...
      if (!params.approvalId) {
        ...
        throw new LocalBridgeError(
          LocalBridgeErrorCode.APPROVAL_REQUIRED,
          `Operation requires human approval. Approval request "${approval.id}" created for modifying "${params.path}". Please ask the user to review and approve in LocalBridge Desktop, check status with localbridge_approval_status(approvalId: "${approval.id}"), and retry with approvalId: "${approval.id}".`,
          {
