    ...
    if (evalResult.decision === "ask") {
      if (!params.approvalId) {
        ...
        throw new LocalBridgeError(
          ...
          `Operation requires human approval. Approval request "${approval.id}" created for switching to branch "${branchName}". Please review and approve in Nexus Desktop, and retry with approvalId: "${approval.id}".`,
          {
