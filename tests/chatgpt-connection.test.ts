...
describe("ChatGPT Dedicated Connection Suite", () => {
  ...
  it("exclusively registers ChatGPTConnectorAdapter in AdapterRegistry with 59 tools", async () => {
    ...
    expect(health.toolCount).toBe(59);

    // Test connection
    const testRes = await adapter!.testConnection();
    expect(testRes.success).toBe(true);
    expect(testRes.toolCount).toBe(59);
  });
