...
describe("MCP Tools Production Count Parity (62 Tools)", () => {
  ...
  it("MCP_TOOL_SCOPE policy table defines exactly 62 tools", () => {
    const totalTools = Object.keys(MCP_TOOL_SCOPE).length;
    expect(totalTools).toBe(62);
  ...
  it("GET /api/mcp/status returns dynamic toolsCount equal to 62", async () => {
    const res = await fetch(`http://127.0.0.1:${serverPort}/api/mcp/status`);
    expect(res.status).toBe(200);
    const body = (await res.json()) as any;
    expect(body.toolsCount).toBe(62);
  });

  it("MCP tools/list handler returns exactly 62 registered tools", async () => {
    ...
    expect(body.result.tools.length).toBe(62);
  ...
  it("NexusPulseLoading does not contain hardcoded 'MCP Tools (55)' and defaults to 62", () => {
    ...
    expect(content).toMatch(/MCP Tools \(\$\{toolsCount \?\? 62\}\)/);
  });

  it("Sidebar, ControlPage, OverviewPage, SettingsPage do not hardcode 55 tools and default to 62", () => {
    ...
    expect(sidebar).toContain("toolsCount ?? 62");
    expect(control).toContain("toolsCount ?? 62");
    expect(overview).toContain("toolsCount ?? 62");
  });
});
